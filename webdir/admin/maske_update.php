...
<table>
<form action ="update.php" method="GET" name="Formular" onsubmit="return chkFormular()">
<?php
...
{
$query="DELETE FROM $table where K0i1 = '$id' ";
$result= mysql_query($query);

$query="DELETE FROM k087_p where id = '$id' ";
$result= mysql_query($query);
...
die ("<p align =\"center\">Daten wurden erfolgreich gel�scht <a href =\"javascript:_close ()\">[Fenster schlie�en]</a></p>");
...
if (($var == "K087")|| ($var == "K200")|| ($var == "K204"))$query2="SELECT 'p' as sart,   IFNULL(P.K900,H_k.id_p )  as name,IFNULL(P.K0i1 ,H_k.id_p )as id,P.K0i1 as ch, H_k.K0i1 as H_k_id  FROM $var2 H_k LEFT JOIN  person P ON H_k.id_p = P.K0i1 where H_k.id ='$id' ";
else if ($var = "K441")$query2="SELECT 'z' as sart,IFNULL(IFNULL(F.K640,F.K620),H_k.id_z) as name,IFNULL(F.K0i1 ,H_k.id_z )as id,F.K0i1 as ch, H_k.K0i1 as H_k_id,H_k.K521a,H_k.K521b,H_k.K521c,H_k.K521d,H_k.zeort  FROM $var2 H_k LEFT JOIN  periodica F ON H_k.id_z = F.K0i1 where H_k.id ='$id' ";
...
else if (($var == "K240")||($var == "K630"))                                $query2="SELECT I.K003 as sart,I.K800 as name,IFNULL(I.K0i1,H.$var) as id  FROM $table H LEFT JOIN  institution I ON I.K0i1 = H.$var where H.K0i1 ='$id' ";
$result= mysql_query($query2);
...
?>
<a  href ="javascript:_close ()">[Fenster schlie�en]</a></td>
</form>
</tr></table>
...
function ensure (delete_id,id,sart,delete_mod,delete_table)
{

if (confirm("Loeschen?")== true){
 var Ziel = "maske_update.php?id="+id+"&sart="+sart+"&delete_id="+delete_id+"&delete_mod="+delete_mod+"&delete_table="+delete_table+"";
